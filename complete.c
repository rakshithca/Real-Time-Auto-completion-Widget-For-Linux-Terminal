...
#include<fcntl.h>  // ACwidget - below 3 lines added
#include<sys/types.h>
#include<sys/stat.h>
	      ...
	      else
		{
		  ...
		  if (j + 1 < limit)
		    for (k = 0; k < max - printed_len; k++)
		      putc(' ', rl_outstream);		// Check1 add space
		}
	      l += count;
	    }
	  rl_crlf ();
     ...
     int what_to_do;
{
  ...
  char *temp; // ACwidget
  int len; // ACwidget - To store the number of possibilities
  ...
  saved_line_buffer = rl_line_buffer ? savestring (rl_line_buffer) : (char *)NULL;  // Check1 var1
  ...
  switch (what_to_do)
      ...
      if (rl_completion_display_matches_hook == 0)
	{
      ...
      display_matches (matches);   // ACwidget
      _rl_sigcleanup = 0;
      _rl_sigcleanarg = 0;
      break;
      
    case '~':  // This case has been added for AC widget
      if (rl_completion_display_matches_hook == 0)
	{
	  _rl_sigcleanup = _rl_complete_sigcleanup;
	  _rl_sigcleanarg = matches;
...
// ACwidget changes begin
	// To create the fifo file and open it for the first time
	if(fd==0)
	{
		mkfifo("/home/varun/v2f",S_IFIFO|S_IRWXU|S_IRWXG|S_IRWXO);  // Change the 'varun' to your current ubuntu username 
		fd=open("/home/varun/v2f",O_WRONLY|O_NONBLOCK);		// Change the 'varun' to your current ubuntu username 
	}
	
	// If matches[1] is true, then there are more that 1 possible matches
	if (matches[1])
   	{
   			
   		  // Count the number of matches
   		  for (len = 0,i = 1; matches[i]; i++ , len++)
		  {

   		  }

 		// '#' symbol represents the start of new possibilities
 		write(fd,"#",1);
 		...
 		if(len>20)
	   	...
	   	// Write every string from array to fifo file delimited by 'space'
      		for (i = 1; matches[i]; i++)
		{
			write(fd,matches[i],strlen(matches[i]));
			write(fd," ",1);
		}
	...
	{
		write(fd,"#",1);
		write(fd,matches[0],strlen(matches[0]));
		write(fd," ",1);

	}
	
	write(fd,"^",1);  // write '^' to fifo file to identify the end of copying

// ACwidget changes end
...
// ACwidget - custom function to copy the rl_line_buffer contents when 'space' key is pressed in terminal
int
acwidget_line_copy_1()
{
	if(fd==0)
	{
		mkfifo("/home/varun/v2f",S_IFIFO|S_IRWXU|S_IRWXG|S_IRWXO); // Change the 'varun' to your current ubuntu username 
	...
	write(fd,"(",1);
	write(fd,rl_line_buffer,strlen(rl_line_buffer));
	write(fd,")",1);
